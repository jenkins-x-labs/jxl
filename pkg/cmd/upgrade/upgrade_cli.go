...
func (o *UpgradeCLIOptions) Run() error {
	...
	log.Logger().Debugf("Current version of jxl: %s", util.ColorInfo(currentVersion))
...
func (o *UpgradeCLIOptions) candidateInstallVersion() (semver.Version, error) {
	if o.Version == "" {
		...
		latestVersion, err := version.GetJXLVersion(resolver)
		if err != nil {
			return semver.Version{}, errors.Wrap(err, "failed to determine version of latest jxl release")
		}
...
func (o *UpgradeCLIOptions) NeedsUpgrade(currentVersion semver.Version, latestVersion semver.Version) bool {
	if latestVersion.EQ(currentVersion) {
		log.Logger().Infof("You are already on the latest version of jxl %s", util.ColorInfo(currentVersion.String()))
		return false
	}
	return true
}
